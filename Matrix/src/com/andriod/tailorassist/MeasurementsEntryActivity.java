...
//        	measurementField.setHint(fragment.getCaption());
    ...
    public long addCustomerDetails(Bundle extras){    	
    	...
    	if((custShirt.length()== 0) && (custPant .length()== 0) && (custOther.length()==0)){
    		
    		Toast.makeText(Ctxt, R.string.alertTxt_NewCustomer_Measuremetns_emptyMsg, Toast.
    				LENGTH_LONG).show();
    	}
    	else{
    		Log.d("addCustomerDetails", "shirtDetails : "+custShirt+" trouserDetails : "+custPant+" otherDetails :"+custOther);
...
//		    	String custAddress = extras.getString("custAddress");   	
		    	...
		    	custTable.open();
		    	if(custId<=0){
		    		newCustNo = custTable.addCustomer(custName, custMobile, custAddress, custShirt, custPant, custOther);   	
		    	}else{
		    		if(custTable.updateCustomer(custId, custName, custMobile, custAddress, custShirt, custPant, custOther))
		    		{
		    			newCustNo = custId;
		    		}
		    	}
		    	...
		    	if(newCustNo>0){
		    		...
		    		Toast.makeText(Ctxt, "Failed to save customer details",Toast.LENGTH_SHORT).show();
		    	}
    	}
		return newCustNo;    	
    }
...
//    	measurementField.append(btnTxt + "\t"); 	
    	...
    	MeasurmentFragment msrmntFrg = (MeasurmentFragment)mSectionsPagerAdapter.getItem(mViewPager.getCurrentItem());
    	msrmntFrg.appendMeasurement(btnTxt);
//    	EditText measurementField;    	
//    	measurementField = 	(EditText)msrmntFrg.getText(R.id.editText_measurement);
